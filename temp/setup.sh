...
ESSENTIAL_PACKAGES=(
    ...
    "libx11" "libxft" "libxinerama"  # Added for DWM build
)
...
setup_ufw_firewall() {
    ...
    # Make media rules optional but non-interactive by default (change to 'y' if needed)
    local enable_media_rules="n"
    if [[ $enable_media_rules =~ ^[Yy]$ ]]; then
...
static const unsigned int refreshrate = 60; /* mouse refresh rate in Hz */
...
apply_dwm_patches() {
    ...
    cd "$DWM_DIR"
    # Add patches here if needed for future iterations (e.g., curl -O https://dwm.suckless.org/patches/fullscreen/dwm-fullscreen-6.4.diff; patch -p1 < dwm-fullscreen-6.4.diff)
    log "Skipping patches for simplified setup (can add in next iteration)"
}
...
configure_amd_kernel_parameters() {
    ...
    # Default to 'guided' for non-interactive; change if needed
    local PSTATE_MODE="guided"
...
main() {
    ...
    if [ $# -eq 0 ]; then
        if [[ -t 0 ]]; then
            echo -e "${YELLOW}Continue with interactive mode? (y/N, timeout: 5s)${NC}"
            if read -t 5 -n 1 -r; then
                echo
                if [[ $REPLY =~ ^[Yy]$ ]]; then
                    interactive_menu
                else
                ...
                log "No input received, defaulting to interactive mode"
                interactive_menu
            fi
        else
            log "Non-interactive environment detected, running full setup"
            run_full_setup
        fi
    else
        case $1 in
